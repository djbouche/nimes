...
proc writePalette*(ppu: var PPU, adr: uint16, val: uint8) =
...
proc `control=`(p: var Pulse, val: uint8) =
...
proc `sweep=`(p: var Pulse, val: uint8) =
...
proc `control=`(n: var Noise, val: uint8) =
...
proc `frameCounter=`(apu: var APU, val: uint8) =
  apu.framePeriod = 4'u8 + ((val shr 7) and 1)
  apu.frameIRQ = ((val shr 6) and 1) == 0

proc `control=`(apu: var APU, val: uint8) =
...
proc `[]`*(apu: var APU, adr: uint16): uint8 =
  if adr == 0x4015:
...
proc `[]=`*(apu: var APU, adr: uint16, val: uint8) =
  case adr
...
proc `mask=`*(ppu: var PPU, val: uint8) =
...
proc `scroll=`(ppu: var PPU, val: uint8) =
  if ppu.w == 0:
...
proc `address=`(ppu: var PPU, val: uint8) =
  if ppu.w == 0:
...
proc `data=`(ppu: var PPU, val: uint8) =
  ppu.mem[ppu.v] = val
  ppu.v += (if ppu.flagIncrement: 32 else: 1)

proc `dma=`(ppu: var PPU, val: uint8) =
  template cpu: CPU = ppu.nes.cpu
...
proc `[]`*(ppu: var PPU, adr: uint16): uint8 =
  case adr
...
proc `[]=`*(ppu: var PPU, adr: uint16, val: uint8) =
  ppu.register = val
  case adr
